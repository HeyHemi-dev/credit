...
export async function requireUserId(request: Request): Promise<string> {
  ...
  fetch('http://127.0.0.1:7243/ingest/17037db5-3e4a-43c3-8a86-d6aac6646a48', {
    ...
    body: JSON.stringify({
      ...
      runId: 'run1',
      hypothesisId: 'A',
      location: 'src/lib/server/auth.ts:requireUserId',
      message: 'requireUserId entry',
      data: {
        hasAuthUrl: !!authUrl,
  ...
  ;(() => {
    ...
    fetch('http://127.0.0.1:7243/ingest/17037db5-3e4a-43c3-8a86-d6aac6646a48', {
      ...
      body: JSON.stringify({
        ...
        hypothesisId: 'F',
        location: 'src/lib/server/auth.ts:requireUserId',
        message: 'cookie names summary',
        data: {
          cookieNameCount: cookieNames.length,
          cookieNames,
          looksLikeNeonAuth: joined.includes('neon') || joined.includes('auth'),
          looksLikeSession:
            joined.includes('session') || joined.includes('token'),
        },
        timestamp: Date.now(),
      }),
    }).catch(() => {})
  ...
  fetch('http://127.0.0.1:7243/ingest/17037db5-3e4a-43c3-8a86-d6aac6646a48', {
    ...
    body: JSON.stringify({
      ...
      hypothesisId: 'B',
      location: 'src/lib/server/auth.ts:requireUserId',
      message: 'auth client created',
      data: { hasGetSession: typeof auth.getSession === 'function' },
      timestamp: Date.now(),
    }),
  }).catch(() => {})
  // #endregion
  ...
  fetch('http://127.0.0.1:7243/ingest/17037db5-3e4a-43c3-8a86-d6aac6646a48', {
    ...
    body: JSON.stringify({
      ...
      hypothesisId: 'G',
      location: 'src/lib/server/auth.ts:requireUserId',
      message: 'getSession flags',
      data: {
        hasData: !!data,
        hasError: !!error,
        hasSession: !!(data as any)?.session,
        hasUser: !!(data as any)?.session?.user,
      },
      timestamp: Date.now(),
    }),
  }).catch(() => {})
