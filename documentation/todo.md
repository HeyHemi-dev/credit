...
- [x] Define directory/profile MVP (public profile fields, URL/slug strategy, rating model, what's free vs paid). Decision recorded in `documentation/with-thanks_directory-profile-mvp.md`
- [x] Decide model: 1 user -> 1 supplier, or 1 user -> many suppliers. One to many requires UI for switching between suppliers
...
- [ ] Reorganize serverFn files into `src/server-fns` and `src/server`: keep `src/server` truly server-only, and isolate any `createServerOnlyFn` / dynamic import boundary code to `src/server-fns`
